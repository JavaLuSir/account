...
public class DataAccountServiceImpl implements ServiceDataAccount {
    ...
    public Map<String, String> addDetail(Map<String, String> param) {
        ...
        // 如果前端传TRTYPE，直接使用；否则从TRKIND获取
        if (trtype == null || trtype.isEmpty()) {
            trtype = tradekind.substring(tradekind.length() - 1);
            if (trtype == null || trtype.isEmpty()) {
                trtype = "1";
            }
        }
    ...
    public List<Map<String, Object>> queryKind() {
        String sqlKind = "SELECT DICKEY VALUE,DICVAL TEXT FROM T_DICT WHERE TEAM='A' ORDER BY ORDENUM";
